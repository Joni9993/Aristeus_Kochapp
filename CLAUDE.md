...
- **Design „Honig & Olive":** CSS-Variablen-Tokens in `frontend/src/index.css` (surface/card/ink/muted/line/olive/honey als RGB-Triplets), Fraunces (`@fontsource-variable/fraunces`, lokal) als `font-display` für Titel/Gerichtsnamen, `components/Laurel.tsx`. REGEL: Honig-Gold exklusiv für Angebote/Ersparnis/Favoriten-Sterne, Olivgrün für Primäraktionen, keine `stone-`/`emerald-`-Klassen mehr in src/
- **Design-Umschalter (seit v2.2.1):** `data-theme="light"|"dark"` auf `<html>`, gesteuert von `frontend/src/hooks/useTheme.ts` (System/Hell/Dunkel, Default System, persistiert in localStorage); Pre-Paint-Script in `index.html` verhindert Flash; Tailwind `darkMode: ['selector', '[data-theme="dark"]']`. Umschalter im „Mehr"-Sheet (`BottomNav.tsx`)
- **Foto-Rezepterkennung (seit v2.3.0):** `POST /api/recipes/import-photo` (multipart, bis 4 Bilder) nutzt eine separate Vision-Modell-Chain (`Settings.vision_models`, Default `gemma-4-31b-it:free`/`gemma-4-26b-a4b-it:free`/`nemotron-nano-12b-v2-vl:free` — alle Free-Tier bildfähig, `chat_completion_json(models=...)` überschreibt dafür die normale Chain statt sie zu erweitern). Frontend: `components/PhotoRecipeImport.tsx`, dritte Kachel im „Rezept hinzufügen"-Dialog (Cookbook.tsx)
