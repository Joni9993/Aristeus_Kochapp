    ...
    <>
      ...
      <ul className="mb-4 space-y-1">
        {recipe.zutaten.map((ing, i) => (
          <li key={i} className="flex flex-wrap items-center gap-x-2 gap-y-1 text-stone-600">
            {ing.menge && <span className="shrink-0 font-medium">{ing.menge} {ing.einheit}</span>}
            <span className="min-w-0">{ing.name}</span>
            {ing.ist_angebot && (
              <span className="shrink-0 rounded bg-emerald-100 px-1 text-xs text-emerald-700">Angebot</span>
            )}
          </li>
        ))}
      </ul>
