  ...
  return (
    <div className="mt-4 space-y-3 border-t border-stone-100 pt-3">
      <div className="flex items-center gap-3">
        <span className="shrink-0 text-xs text-stone-400">Wie war's?</span>
        <button
          onClick={() => handleThumbs(1)}
          className={`min-h-11 min-w-11 rounded-full px-3 py-2 text-sm ${thumbs === 1 ? 'bg-emerald-500 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
        >
          👍
        </button>
        <button
          onClick={() => handleThumbs(-1)}
          className={`min-h-11 min-w-11 rounded-full px-3 py-2 text-sm ${thumbs === -1 ? 'bg-red-400 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
        >
          👎
        </button>
        <button
          onClick={handleFav}
          className={`ml-auto shrink-0 p-2 text-lg leading-none ${fav ? 'text-amber-400' : 'text-stone-300 hover:text-stone-400'}`}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="shrink-0 text-xs text-stone-400">Portion:</span>
        {(['zu wenig', 'genau richtig', 'zu viel'] as const).map((opt) => (
          <button
            key={opt}
            onClick={() => handlePortion(opt)}
            className={`min-h-9 rounded-full px-2.5 py-1.5 text-xs ${portion === opt ? 'bg-emerald-500 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            {opt}
          </button>
        ))}
      ...
      <div className="flex gap-2">
        <textarea
          ...
          rows={2}
          className="min-w-0 flex-1 resize-none rounded-lg border border-stone-200 p-2 text-sm focus:border-emerald-400 focus:outline-none"
        />
        <button
          onClick={saveText}
          disabled={saving}
          className="shrink-0 self-end rounded-lg bg-stone-100 px-3 py-2 text-xs text-stone-600 hover:bg-stone-200 disabled:opacity-50"
        >
